...
export default function ViewerCore({
  ...
  if (isLoading) {
    return <p className="text-gray-400 text-center mt-6 text-sm">문제를 불러오는 중입니다...</p>;
  }
  ...
  if (filteredSubjects.length === 0 && !isLoading) {
    return <div className="flex-1 flex items-center justify-center min-h-[300px]"><EmptyMessage message="선택하신 조건에 해당하는 문제가 없습니다." /></div>;
  }
        ...
        <AnimatePresence mode="wait">
          {selectedBlock ? (
            ...
            >
              ...
              <div className="flex flex-row justify-center items-center gap-3 mt-8">{footerContent}</div>
            </motion.section>
          ) : (
            !isLoading && <div className="flex-1 flex items-center justify-center min-h-[300px]"><EmptyMessage /></div>
          )}
        </AnimatePresence>
      </main>
    </div>
